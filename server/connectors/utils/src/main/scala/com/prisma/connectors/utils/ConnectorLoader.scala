...
import com.prisma.api.connector.native.ApiConnectorNative
...
object ConnectorLoader {
  def loadApiConnector(config: PrismaConfig)(implicit ec: ExecutionContext, drivers: SupportedDrivers): ApiConnector = {
    ...
    databaseConfig.connector match {
      ...
      case "sqlite-native"            => ApiConnectorNative(databaseConfig)
      case "postgres-native"          => ApiConnectorNative(databaseConfig)
      case "native-integration-tests" => ApiConnectorNative(databaseConfig)
